...
The acquisition ledger is separate from the closed spike ledger. Status begins from the documented three spike successes without reading that historical file. The internal target is 120 combined successes: three existing plus up to 117 useful acquisition successes. The current Academy requirement says 100+ while the older campaign page says 1,000; both remain recorded until reconciled. Dashboard observations are 1,092 after the closed spike, 1,095 immediately before the later six-call pilot, and 1,086 afterward. They are separate manual observations; no reconciliation or current balance is inferred.

The bounded acquisition workflow subsequently made six discovery calls under prior authorization. Version 3 rejected all 600 returned rows, so that live run produced no candidate and made no candidate-specific coverage request. Offline version 4 reprocessing later identified one qualifying row and one distinct candidate and planned one coverage work item; that does not retroactively create a live coverage call or validate `filters.trader_address`. The current CLI accepts only digits `1`–`10` for a live per-run allowance, but no further live call is authorized by implementation, credential availability, cached evidence, diagnosis, or reprocessing. Separate clones/worktrees cannot coordinate the local lock or ledger and must not run live acquisition.
...
The exact offline reprocessing command ran once against the canonical private evidence. It verified six version 3 cache pages against deterministic work, source fingerprints, request IDs, and six settled successful ledger attempts; preserved the original version 3 state/cache; and wrote separate version 4 derivatives with private source-attempt provenance. Sanitized output reported 600 valid whole-second rows, one qualifying row, one candidate, and one planned coverage item, with zero new calls, attempts, successes, or credits. Keyless status remained six attempts/successes/reported credits/retained credits and nine combined successes. This is cached discovery evidence only: it does not establish eligibility, complete lookback/answer coverage, direction semantics, stable leg identity, or publishable historical data.
