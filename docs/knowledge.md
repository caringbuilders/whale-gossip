...
The acquisition ledger is separate from the closed spike ledger. Combined reporting begins with the documented three spike successes without importing that historical file. Acquisition is now frozen at exactly 100 combined successes: three spike plus 97 acquisition. The current Academy requirement says 100+ while the older campaign page says 1,000; organizer reconciliation remains necessary.

The original pre-spike balance of 1,095 is explained by the three spike credits: `1,095 − 3 = 1,092`. A separate later pre-discovery reading also displayed 1,095 when 1,092 was expected; only that later reading remains unexplained/transient. The post-discovery 1,086 equals `1,092 − 6`. Final reconciliation is `1,092 − 97 = 995`, matching reported acquisition credits.

The bounded workflow first made six discovery calls under prior authorization. Version 3 rejected all 600 returned rows; offline version 4 reprocessing later identified one qualifying row and one private candidate. Subsequent supplied aggregate evidence records 91 successful candidate-specific coverage calls with consistent wallet-filter enforcement, but page 91 remained nonterminal. Each call requested a new sequential page for the proposed 32-day window rather than repeating an identical request. Early pages established filter enforcement and extreme activity density. Once completion looked unlikely within budget, the user separately authorized continued sequential coverage to the chosen Academy stop condition. Later pages added limited diversity beyond continuing wallet/token consistency and density evidence. The candidate lacks complete coverage and cannot become a real round. No further discovery or coverage call is authorized.
