...
The endpoint has not supplied a provider-stable leg ID. `derived-v1` IDs hash the canonical complete row for deterministic local processing; that label remains the ID format under adapter version 3. A provider label can therefore affect the private fingerprint, but labels are not copied into normalized events or reports. Candidate coverage now rejects exact and cross-page duplicates instead of collapsing them, because repeated subthreshold rows could otherwise produce a false No trade. Conflicting rows and multiple matching rows under one transaction hash also reject conservatively while correction and leg semantics are unknown.
...
The bounded acquisition workflow subsequently made six discovery calls under prior authorization. It returned 600 rows but no candidate passed current normalization, so no candidate-specific coverage request occurred and `filters.trader_address` remains unverified. The current CLI accepts only digits `1`–`10` for a live per-run allowance, but no further live call is authorized by implementation, credential availability, cached evidence, or this diagnostic. Separate clones/worktrees cannot coordinate the local lock or ledger and must not run live acquisition.
...
The documented provider response schema identifies `block_timestamp` as a timestamp string but does not guarantee exact-millisecond fractional precision. The observed whole-second shape is evidence for a future normalization decision, not permission to loosen the boundary now. Any change must define how whole-second observations interact with exact cutoff and 48-hour boundaries and requires its own review and regression tests.
