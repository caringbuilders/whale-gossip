...
The first acquisition design used token-wide coverage and could spend an entire pilot draining one dense discovery window. Claude rejected that design before any acquisition call occurred. Adapter/state/schema version 3 treats discovery as bounded sampling: three six-hour periods, page 1 spread across them, at most two pages per period, and at most six discovery calls. Candidate extraction happens on each valid page without waiting for terminal pagination. A capped sample is never completeness evidence.

Corrected coverage pairs the fixed token with a provisional `filters.trader_address` field using the normalized wallet obtained from validated discovery, and uses a unique candidate ID so wallets sharing a cutoff remain independent. No tracked project source independently confirms this request field. It has not been exercised live. Private pilot evidence must establish that the provider accepts it, enforces it on every page, paginates stably, and permits complete intervals within the budget.

Inclusive provider dates are handled by a one-millisecond request overlap at the local start and inclusion of the local exclusive end. The adapter then enforces `[start, end)` locally. This is a defensive transformation, not live proof of Nansen's boundary behavior. Adapter version 4 accepts only canonical whole-second UTC or exact three-digit millisecond UTC. A whole-second source is represented internally with `.000Z` at the same instant and retains `whole-second` precision metadata; the zeros are canonical form, not invented provider precision.

The endpoint has not supplied a provider-stable leg ID. `derived-v1` IDs hash the canonical complete row for deterministic local processing; that label remains the ID format under adapter version 4. Whole-second and exact `.000Z` representations of the same otherwise identical row canonicalize to the same private derived ID. A provider label can therefore affect the private fingerprint, but labels are not copied into normalized events or reports. Candidate coverage rejects exact and cross-page duplicates instead of collapsing them, because repeated subthreshold rows could otherwise produce a false No trade. Conflicting rows and multiple matching rows under one transaction hash also reject conservatively while correction and leg semantics are unknown.
...
The acquisition ledger is separate from the closed spike ledger. Combined reporting begins with the documented three spike successes without importing that historical file. Acquisition is now frozen at exactly 101 combined successes: three spike plus 98 acquisition. The newer detailed Academy FAQ says 100+, while the campaign landing page still says 1,000; only Nansen can definitively reconcile the official-source conflict.

The original pre-spike balance of 1,095 is explained by the three spike credits: `1,095 − 3 = 1,092`. A separate later pre-discovery reading also displayed 1,095 when 1,092 was expected; only that later reading remains unexplained/transient. The post-discovery 1,086 equals `1,092 − 6`. The preceding acquisition run reconciles as `1,092 − 97 = 995`, matching its 97 reported credits. After the one-credit recording call, the dashboard displayed 1,004, a net increase of nine from 995. Treat 1,004 only as an unexplained manual observation: do not alter the ledger, infer a grant or balance history, or claim the final call reconciled through the dashboard.

The bounded workflow first made six discovery calls under prior authorization. Version 3 rejected all 600 returned rows; offline version 4 reprocessing later identified one qualifying row and one private candidate. Supplied aggregate evidence now records 92 successful candidate-specific coverage calls with consistent wallet-filter enforcement, but page 92 remained nonterminal. Each call requested a new sequential page for the proposed 32-day window rather than repeating an identical request. Early pages established filter enforcement and extreme activity density. Once completion looked unlikely within budget, the user separately authorized continued sequential coverage to the chosen Academy stop condition. Later pages added limited diversity beyond continuing wallet/token consistency and density evidence. The final one-call recording run requested page 92 and stopped at its per-run limit. The candidate lacks complete coverage and cannot become a real round. No further discovery or coverage call is planned or authorized.

The keyless `--diagnose-cache` mode classified the six canonical discovery cache entries without a network request, credential read, lock, or persistent write. All 600 timestamps were ISO UTC strings at whole-second precision, while none used the adapter's required exact three-digit milliseconds. All 600 USD fields were positive finite numbers; none were strings, null, missing, negative, zero, or another invalid type. The diagnostic therefore separated the prior combined rejection into 600 timestamp-only failures and zero value failures. Every row had a valid wallet address, matching configured token address, valid transaction hash, and BUY/SELL action, so all 600 would otherwise pass those required discovery field checks. The diagnostic did not expose exact timestamps, values, identities, hashes, labels, or raw rows and did not evaluate admission/candidate timing after normalization.

The documented provider response schema identifies `block_timestamp` as a timestamp string but does not guarantee exact-millisecond fractional precision. The narrow version 4 rule follows the observed whole-second shape without accepting offsets, arbitrary fractional precision, impossible dates, whitespace, or other parseable variants. Boundary regressions confirm lower-bound inclusion, upper-bound exclusion, and deterministic rejection when multiple material events share the earliest canonical second.

The exact offline reprocessing command ran once against the canonical private evidence. It verified six version 3 cache pages against deterministic work and settled attempt provenance; preserved the source evidence; and wrote separate version 4 derivatives. Sanitized output reported 600 valid whole-second rows, one qualifying row, one candidate, and one planned coverage item, with zero new calls, attempts, successes, or credits. Its nine-combined-success status was the historical point immediately after reprocessing, before the later 91 supplied coverage successes. Reprocessing alone did not establish eligibility or a publishable round.

Reusable acquisition lessons from the completed run:

- Timestamp precision must be observed and retained explicitly. Canonical `.000Z` representation of a whole-second source must not be described as measured milliseconds.
- Dense pagination can consume a bounded budget without reaching a terminal page. A high page count and consistent filter matches still do not prove complete coverage.
- Durable per-attempt accounting and a predetermined stop condition made it possible to freeze honestly at the chosen threshold with no unknown charges or failures.
- Calls-for-count remain a poor default. If deadline pressure leads to an explicit exception, preserve sequential request provenance and disclose when later calls provide diminishing informational value rather than describing every call as equally useful.
- Public reporting should expose allowlisted counts and limitations while keeping wallet identity, raw responses, and exact trade evidence private.
...
| Item | Evidence |
| --- | --- |
| Project directory | Repository root of the working checkout |
...
- Which attempts Nansen will count toward competition eligibility remains provider-dependent. The project records 101 successful calls and proceeds under the newer detailed FAQ's 100+ wording, while the campaign landing page still says 1,000. Only Nansen can definitively reconcile the official-source conflict; planned requests and cache hits do not count as observed usage.
