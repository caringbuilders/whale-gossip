...
Current milestone: Claude's blocking review findings for the offline Nansen acquisition workflow are corrected and offline-tested for re-review. The corrected workflow has not been executed live. The accepted five-round synthetic game remains the public experience, and real/live acquisition stays disabled pending independent re-review and a separate 10-call authorization.
...
- Adapter/state/schema version 2 replaces the rejected discovery design. Discovery is three deterministic, non-overlapping six-hour WETH samples, spread across all windows and capped at two pages each. Candidate rows are processed after every valid page; capped windows are labelled sampled, never complete. A no-candidate nonterminal run stops after six calls instead of draining one window.
- Candidate coverage is independent and wallet-filtered with the repository-reviewed `filters.trader_address` field populated only from validated discovery. Its identity includes the candidate and wallet. Two candidates sharing a cutoff remain separate work/results. This request shape has not been live-verified.
...
- **Codex verification for the correction:** all eight test files passed under the automatic outbound-network guard after exercising the six-call no-candidate stop, a corrected ten-call schedule with three discovery and seven candidate-coverage calls, two candidates sharing one cutoff, cache hits with zero fetches/successes, exact and cross-page duplicates, one-millisecond coverage mutations, malformed state, cross-process scratch locking, direct ledger caps/unknown charges, report privacy, and directory-sync failure before fetch. `npm run lint`, `npm run typecheck`, and `git diff --check` passed. The credential-pattern, tracked-private-data/ignore, and application dependency-boundary scans passed; `package.json` and `package-lock.json` are unchanged. The default `npm run nansen:acquire` dry-run reported `networkRequestSent: false`. Keyless status behavior was exercised only with temporary dummy roots in tests, so no real ledger was opened.
...
Claude's supplied review of `803ba5c` and this correction's disposition are recorded in `docs/reviews/803ba5c.md`. Independent re-review of the correction commit is the next checkpoint. Only after acceptance may a separate authorization permit exactly the documented 10-call pilot. No real round should be published until private acquisition evidence, provider semantics, and the public serialization boundary are reviewed.
