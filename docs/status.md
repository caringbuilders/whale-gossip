...
Current milestone: a resumable bounded Nansen acquisition workflow is implemented and offline-tested for independent review. It has not been executed live. The accepted five-round synthetic game remains the public experience, and real/live acquisition stays disabled pending review and a separate 10-call authorization.
...
- Discovery uses deterministic non-overlapping token windows and never counts as coverage. Candidate coverage retrieves the entire overlapped provider interval for the rules-v4 half-open 30-day lookback and 48-hour answer window, requires exact sequential terminal pagination, reapplies local boundaries, and sorts events locally.
- Adapter-local IDs are canonical full-row SHA-256 fingerprints labelled `derived-v1`. Exact canonical rows may collapse; conflicting row identities, ambiguous repeated transaction hashes, timestamp-precision loss, invalid addresses/hashes/actions/USD values, and incomplete pagination reject a candidate. BUY/SELL remains provisional provider-reported direction.
- Only complete normalized evidence reaches `compileRound`; incomplete evidence cannot become Buy, Sell, or No trade. Raw responses, cache pages, wallets, exact evidence, compiled private answers, and provenance stay under ignored `0600`/`0700` paths. The private manifest records request IDs/pages and versions. A separate aggregate report contains counts only.
- A new acquisition ledger/lock is separate from the permanently closed spike ledger. It caps new attempts and retained/reported credits at 130, reserves before fetch, stops on unexpected or missing pricing, retains unknown charges, throttles to about two requests per second, and permits at most one bounded retry per request. Separate clones/worktrees cannot coordinate and are forbidden for live acquisition.
...
- Dry-run and keyless status are network-free and non-mutating. The future first pilot is limited to ten new calls and requires independent review plus new explicit live authorization. Detailed operations and failure rules are in `docs/nansen-acquisition-runbook.md`.
- **Codex offline verification:** `npm test` passed 136/136 with the automatic network guard; `npm run lint`, `npm run typecheck`, and `git diff --check` passed. The standalone `npm run nansen:acquire` dry-run returned the sanitized WETH-only plan with `networkRequestSent: false`. Tests use temporary roots, dummy keys, and synthetic provider responses to exercise caps, reservations, retries, locks, cache resumption, strict pagination, normalization, compilation, privacy, and application dependency boundaries.
- This implementation made **zero Nansen or other external application calls** and consumed **zero credits**. It did not read `.env.local`, the closed spike ledger, prior raw responses, or other real private evidence. No packages were installed, no production build ran, and nothing was deployed.

Independent review of this named offline implementation is the next checkpoint. After accepted findings are resolved, a separate authorization may permit exactly the documented 10-call pilot. No real round should be published until private acquisition evidence, provider semantics, and the public serialization boundary are reviewed.
...
Independently re-review the named hardening commit, with emphasis on alternate-answer serialization, client response races, server-only enforcement, the policy wording, and a real browser keyboard/mobile flow when browser tooling is available. Real-round acquisition remains a separate later workflow with new review and explicit spending authorization; the closed contract-spike script must not be extended or rerun.
