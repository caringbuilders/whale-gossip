...
- Keep the reviewed token universe at canonical WETH9 until more non-stable ERC-20 contracts are verified without guessing. Candidate-specific coverage provisionally sends `filters.trader_address` with a wallet obtained from validated discovery. No tracked project source independently confirms this request field; provider acceptance and enforcement remain unverified until a live pilot.
...
- Preserve the exact-millisecond timestamp rule until the whole-second pilot observation and cutoff-boundary implications receive independent review. The response schema's timestamp field does not itself guarantee fractional precision. No additional live acquisition is authorized by the six-call pilot or this diagnostic.
