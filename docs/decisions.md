...
- Keep the reviewed token universe at canonical WETH9 until more non-stable ERC-20 contracts are verified without guessing. The endpoint schema identified in the supplied review permits `filters.trader_address`; adapter version 2 uses it only for candidate-specific coverage with a wallet obtained from validated discovery. Its live behavior remains unverified.
...
Use ordinary `tgm/dex-trades` with the fixed token, `only_smart_money=false`, no Smart Money label filters, and no displayed trader labels. Discovery is token sampling; candidate coverage also supplies the repository-reviewed `filters.trader_address`. A bounded probe confirmed the basic response shape but never exercised that wallet filter and returned label fields; the adapter must discard label contents from public and fixture output. Direction semantics, wallet-filter behavior, stable leg identity, complete pagination, and redistribution review remain unresolved. Attribute “Powered by Nansen API”. Keep wallet identities and raw responses private; publish reviewed transformed fixtures only.
