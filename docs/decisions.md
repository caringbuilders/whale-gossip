...
- Keep the reviewed token universe at canonical WETH9 until more non-stable ERC-20 contracts are verified without guessing. Candidate-specific coverage sends `filters.trader_address` with a wallet obtained from validated discovery. Supplied aggregate evidence says the filter was consistently enforced across 92 coverage pages for one candidate; wider provider behavior and terminal completeness remain unverified.
...
- Freeze acquisition at exactly 101 combined successes: three contract-spike plus 98 acquisition successes. The first 100 calls met the newer detailed FAQ's 100+ wording subject to organizer reconciliation; one final user-authorized call requested sequential coverage page 92 for the submission recording. Page 92 remained nonterminal and did not produce a real round. The campaign page's 1,000-call wording remains unresolved. The general rule remains that calls should not be made merely to inflate a count. Record the deadline-bound exception candidly; it is not a precedent that threshold-seeking calls are inherently useful.
...
- No additional discovery or coverage acquisition is authorized after the 101-success freeze. The explicitly user-authorized recording workflow used its one-call allowance on coverage page 92. No further Nansen call is planned.
...
The proposal targets ten real reviewed rounds and one complete live flow first, then deck expansion and hardening. Its internal submission target is September 26, 2026 at 23:59 America/New_York. Its 1,050-call plan was based on the [campaign landing page](https://nansen.ai/campaigns/meridian-buildathon), which still says “Make 1,000 API calls,” and remains historical context. The newer, detailed [Nansen Academy FAQ](https://academy.nansen.ai/articles/3540155-nansen-meridian-buildathon-sep-14-27), shown as updated one day before the final evidence checkpoint, says “Log 100+ API calls between Sep 14th–27th.” The official sources conflict. The project has logged 101 successful calls and proceeds under the newer, detailed FAQ; only Nansen can definitively reconcile the discrepancy. Do not generate wasteful calls to inflate counts.
